...
DEFAULT_PROFILE_DIR = DEFAULT_PROFILE_PATH.parent
...
class ThresholdProfile:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "ThresholdProfile":
        ...
        # Backward/robust compatibility:
        # accept sparse profile JSON (for example files that only contain metadata/timestamp),
        # fill missing required fields from default profile.
        freq_seed: Sequence[float] = DEFAULT_FREQS
        raw_freqs = payload.get("freqs")
        if isinstance(raw_freqs, (list, tuple)) and len(raw_freqs) == 4:
            try:
                freq_seed = tuple(float(value) for value in raw_freqs)
            except Exception:
                freq_seed = DEFAULT_FREQS

        base = asdict(default_profile(freq_seed))
        data = {key: base[key] for key in names if key in base}
        data.update({key: payload[key] for key in names if key in payload})
