...
class RobotExecutor:
    ...
    def _begin_pick_world_legacy_impl(self, x: float, y: float) -> WorldPickPlan:
        with self._lock:
            ...
            target_x = float(x)
            target_y = float(y)
            if target_x < self.limits.x_min or target_x > self.limits.x_max:
                overflow = min(abs(target_x - self.limits.x_min), abs(target_x - self.limits.x_max))
                raise RobotCommandError(
                    RobotErrorCode.TARGET_OUT_OF_WORKSPACE,
                    f"Target exceeds workspace by {overflow:.2f} mm.",
                )
            if target_y < self.limits.y_min or target_y > self.limits.y_max:
                overflow = min(abs(target_y - self.limits.y_min), abs(target_y - self.limits.y_max))
                raise RobotCommandError(
                    RobotErrorCode.TARGET_OUT_OF_WORKSPACE,
                    f"Target exceeds workspace by {overflow:.2f} mm.",
                )
...
class RobotTcpGateway:
    ...
    def dispatch_command(self, line: str, stream) -> None:
        ...
        try:
            ...
            if command == "ABORT":
                if len(parts) != 1:
                    raise ValueError("ABORT takes no arguments")
                self.executor.abort()
            ...
            if command == "MOVE":
                ...
                self.executor.legacy_kernel.move_xy(float(parts[1]), float(parts[2]))
                self._send_line(stream, "ACK MOVE")
                return

            if command == "MOVE_CYL":
                if len(parts) != 4:
                    raise ValueError("MOVE_CYL requires theta r z")
                self.executor.cylindrical_kernel.move_cyl(float(parts[1]), float(parts[2]), float(parts[3]))
                self._send_line(stream, "ACK MOVE")
                return

            if command == "MOVE_CYL_AUTO":
                if len(parts) != 3:
                    raise ValueError("MOVE_CYL_AUTO requires theta r")
                self.executor.cylindrical_kernel.move_cyl_auto(float(parts[1]), float(parts[2]))
                self._send_line(stream, "ACK MOVE")
                return

            if command == "PICK":
                if len(parts) != 3:
                    raise ValueError("PICK requires pixel_x pixel_y")
                plan = self.executor.legacy_kernel.begin_pick(float(parts[1]), float(parts[2]))
            ...
            if command == "PICK_WORLD":
                if len(parts) != 3:
                    raise ValueError("PICK_WORLD requires x y")
                plan = self.executor.legacy_kernel.begin_pick_world(float(parts[1]), float(parts[2]))
                self._send_line(stream, "ACK PICK_STARTED")
                threading.Thread(
                    target=self._pick_worker,
                    args=(plan, stream),
            ...
            if command == "PICK_CYL":
                if len(parts) != 3:
                    raise ValueError("PICK_CYL requires theta r")
                plan = self.executor.cylindrical_kernel.begin_pick_cyl(float(parts[1]), float(parts[2]))
                self._send_line(stream, "ACK PICK_STARTED")
                threading.Thread(
                    target=self._pick_worker,
                    args=(plan, stream),
            ...
            if command == "PLACE":
                if len(parts) != 1:
                    raise ValueError("PLACE takes no arguments")
