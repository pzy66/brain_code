...
| 项目 | 代码落地点 | 说明 |
|---|---|---|
| CLI 参数 | `apps/training_evaluation_ui.py::build_parser`、`tools/training_evaluation_cli.py::build_parser` | 当前主线使用 `--dataset-root` 与 `--include-manifests`；默认数据目录为 `artifacts/datasets`。 |
...
| UI 参数传递 | `apps/training_evaluation_ui.py::TrainingEvaluationWindow`、`tools/training_evaluation_cli.py::main` | UI / CLI 将所选 manifest、run 目录与 profile / report 输出路径统一传入主线训练评测链路。 |
