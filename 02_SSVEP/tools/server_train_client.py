...
REMOTE_CODE_DIR = f"{REMOTE_ROOT}/code/ssvep_mainline"
...
def build_train_command(
    *,
    ...
    task_dir = safe_session_id(task)
    report_dir = assert_remote_ssvep_path(posixpath.join(REMOTE_REPORT_ROOT, task_dir, date_dir, run_id))
    log_path = assert_remote_ssvep_path(posixpath.join(REMOTE_LOG_DIR, f"{run_id}.log"))
    output_profile = assert_remote_ssvep_path(posixpath.join(report_dir, "profile.json"))
    ...
    script = " ".join(args)
    shell_command = (
        f"mkdir -p {sh_quote(report_dir)} {sh_quote(REMOTE_LOG_DIR)} {sh_quote(REMOTE_TMP_DIR)} && "
...
def download_results(ssh: SSHClient, record: dict[str, Any]) -> dict[str, Any]:
    ...
    report_dir = assert_remote_ssvep_path(str(record.get("report_dir", "")))
    remote_rel = posixpath.relpath(report_dir, REMOTE_REPORT_ROOT)
    remote_parts = [part for part in str(remote_rel).split("/") if part and part != "."]
    if len(remote_parts) >= 3:
        local_run_dir = LOCAL_SERVER_RUNS_DIR.joinpath(*remote_parts[-3:])
    else:
    ...
    profile_v2_path = local_run_dir / "profile_v2.json"
    if profile_path.exists():
        published = publish_deployed_profile(
            source_profile=profile_path,
            source_profile_v2=profile_v2_path if profile_v2_path.exists() else None,
            run_dir=local_run_dir,
            task=str(record.get("task", "remote")),
            run_tag=run_id,
            report_json=local_run_dir / "report.json",
            destination_root=LOCAL_SERVER_PROFILES_DIR,
            extra_metadata={
                "source": "remote_download",
                "remote_report_dir": report_dir,
            },
        )
        downloaded_profile = str(published.get("profile_json", ""))
    return {
