...
TDCA_LOCAL_OPT_DELAY_STEPS = (2, 3, 4, 5)
TDCA_LOCAL_OPT_N_COMPONENTS = (2, 3, 4)
TDCA_LOCAL_OPT_COMPUTE_BACKEND = "auto"
...
FBCCA_EXTERNAL_REPLAY_OUTER_EVAL = DEFAULT_FBCCA_EXTERNAL_OUTER_EVAL
FBCCA_EXTERNAL_REPLAY_SPEED = DEFAULT_FBCCA_EXTERNAL_REPLAY_SPEED
...
def _apply_tdca_local_opt_args(args: argparse.Namespace, provided_options: set[str]) -> None:
    args.task = "tdca-local-opt"
    _set_if_missing(args, provided_options, "--models", "models", ",".join(TDCA_LOCAL_OPT_MODELS))
    _set_if_missing(args, provided_options, "--channel-modes", "channel_modes", ",".join(TDCA_LOCAL_OPT_CHANNEL_MODES))
    _set_if_missing(args, provided_options, "--multi-seed-count", "multi_seed_count", int(TDCA_LOCAL_OPT_MULTI_SEED_COUNT))
    _set_if_missing(
        args,
        provided_options,
        "--win-candidates",
        "win_candidates",
        ",".join(f"{float(value):g}" for value in TDCA_LOCAL_OPT_WIN_CANDIDATES),
    )
    _set_if_missing(
        args,
        provided_options,
        "--tdca-delay-steps",
        "tdca_delay_steps",
        ",".join(str(int(value)) for value in TDCA_LOCAL_OPT_DELAY_STEPS),
    )
    _set_if_missing(
        args,
        provided_options,
        "--tdca-n-components",
        "tdca_n_components",
        ",".join(str(int(value)) for value in TDCA_LOCAL_OPT_N_COMPONENTS),
    )
    _set_if_missing(args, provided_options, "--search-preset", "search_preset", str(TDCA_LOCAL_OPT_SEARCH_PRESET))
    _set_if_missing(args, provided_options, "--compute-backend", "compute_backend", str(TDCA_LOCAL_OPT_COMPUTE_BACKEND))
    _set_if_missing(args, provided_options, "--control-state-mode", "control_state_mode", "frequency-specific-logistic")


def _apply_fbcca_local_opt_args(args: argparse.Namespace, provided_options: set[str]) -> None:
    args.task = "fbcca-local-opt"
    _set_if_missing(args, provided_options, "--models", "models", ",".join(FBCCA_LOCAL_OPT_MODELS))
    _set_if_missing(args, provided_options, "--channel-modes", "channel_modes", ",".join(FBCCA_LOCAL_OPT_CHANNEL_MODES))
    _set_if_missing(args, provided_options, "--multi-seed-count", "multi_seed_count", int(FBCCA_LOCAL_OPT_MULTI_SEED_COUNT))
    _set_if_missing(
        args,
        provided_options,
        "--win-candidates",
        "win_candidates",
        ",".join(f"{float(value):g}" for value in FBCCA_LOCAL_OPT_WIN_CANDIDATES),
    )
    _set_if_missing(args, provided_options, "--search-preset", "search_preset", str(FBCCA_LOCAL_OPT_SEARCH_PRESET))
    _set_if_missing(args, provided_options, "--compute-backend", "compute_backend", str(FBCCA_LOCAL_OPT_COMPUTE_BACKEND))
    _set_if_missing(args, provided_options, "--control-state-mode", "control_state_mode", "frequency-specific-logistic")


def _apply_fbcca_external_replay_args(args: argparse.Namespace, provided_options: set[str]) -> None:
    args.task = "fbcca-external-replay-opt"
    _set_if_missing(args, provided_options, "--models", "models", "fbcca")
    _set_if_missing(args, provided_options, "--channel-modes", "channel_modes", "all8")
    _set_if_missing(args, provided_options, "--compute-backend", "compute_backend", str(FBCCA_EXTERNAL_REPLAY_COMPUTE_BACKEND))
    _set_if_missing(args, provided_options, "--search-preset", "search_preset", str(FBCCA_EXTERNAL_REPLAY_SEARCH_PRESET))
    _set_if_missing(args, provided_options, "--control-state-mode", "control_state_mode", "frequency-specific-logistic")
    _set_if_missing(
        args,
        provided_options,
        ...
        Path(DEFAULT_FBCCA_EXTERNAL_DATASET_ROOT),
    )
    _set_if_missing(args, provided_options, "--outer-eval", "outer_eval", str(FBCCA_EXTERNAL_REPLAY_OUTER_EVAL))
    _set_if_missing(args, provided_options, "--replay-speed", "replay_speed", str(FBCCA_EXTERNAL_REPLAY_SPEED))
    _set_if_missing(
        args,
        provided_options,
        "--output-profile",
        "output_profile",
        Path(DEFAULT_PROFILE_PATH).with_name("profile.json"),
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--report-path", type=Path, default=DEFAULT_REPORT_ROOT / "report.json")
    ...
    parser.add_argument("--tdca-delay-steps", type=str, default=",".join(str(v) for v in TDCA_LOCAL_OPT_DELAY_STEPS))
    parser.add_argument("--tdca-n-components", type=str, default=",".join(str(v) for v in TDCA_LOCAL_OPT_N_COMPONENTS))
    parser.add_argument(
        "--search-preset",
        type=str,
    ...
    parser.add_argument(
        "--outer-eval",
        type=str,
        default=str(FBCCA_EXTERNAL_REPLAY_OUTER_EVAL),
        choices=list(FBCCA_EXTERNAL_OUTER_EVALS),
    )
    parser.add_argument(
        "--replay-speed",
        type=str,
        default=str(FBCCA_EXTERNAL_REPLAY_SPEED),
        choices=list(FBCCA_EXTERNAL_REPLAY_SPEEDS),
    )
    ...
    parser.add_argument(
        ...
        help="tdca-local-opt compares the repository's legacy TDCA-like decoder and a paper-aligned TDCA variant within the same async optimization flow.",
        choices=[
            "fbcca-weights",
            "model-compare",
            "fbcca-weighted-compare",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if task != "fbcca-external-replay-opt" and session1 is None:
        raise ValueError("--dataset-manifest or --include-manifests is required")

    if task == "tdca-local-opt":
        local_config = TDCALocalOptConfig(
            ...
            tdca_delay_steps=tuple(int(float(item.strip())) for item in str(args.tdca_delay_steps).split(",") if item.strip()),
            tdca_n_components=tuple(int(float(item.strip())) for item in str(args.tdca_n_components).split(",") if item.strip()),
        ...
        run_tdca_local_opt(local_config, log_fn=lambda text: print(text, flush=True))
        return 0
    if task == "fbcca-local-opt":
        ...
        run_fbcca_local_opt(local_config, log_fn=lambda text: print(text, flush=True))
        return 0
    if task == "fbcca-external-replay-opt":
        ...
        run_fbcca_external_replay_opt(external_config, log_fn=lambda text: print(text, flush=True))
        return 0
