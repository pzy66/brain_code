...
def _resolve_report_paths(
    ...
    *,
    ...
    task_name = str(config.task or DEFAULT_TRAIN_EVAL_TASK)
    subject_tag = sanitize_artifact_token(subject_id, fallback="mixed")
    run_tag = make_run_tag(task=f"{task_name}-{subject_tag}")
...
def _load_session1_dataset(
    ...
    merged_dataset = LoadedDataset(
        ...
        npz_path=reference.npz_path,
            session_id=f"merged_{sanitize_artifact_token('_'.join(session_ids[:3]), fallback='sessions')}_{len(datasets)}",
    ...
    *,
    ...
    if chosen_profile_obj is not None:
        ...
        publish_deployed_profile(
            ...
            source_profile_v2=profile_v2_output if profile_v2_saved else None,
            run_dir=report_dir,
            task=str(config.task or DEFAULT_TRAIN_EVAL_TASK),
            run_tag=run_tag,
