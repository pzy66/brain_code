...
DEFAULT_TDCA_DECODER_VARIANT = "tdca_like_legacy"
TDCA_DECODER_VARIANTS = ("tdca_like_legacy", "tdca_paper_aligned")
DEFAULT_TDCA_TRAINING_LATENCY_SEC = 0.14
DEFAULT_BENCHMARK_DATASET_ROOT = DATASETS_DIR
...
MODEL_IMPLEMENTATION_LEVELS = {
    "cca": "paper-faithful",
    "cca_itcca_combo": "paper-faithful",
...
MODEL_METHOD_NOTES = {
    ...
    "cca_itcca_combo": "CCA + IT-CCA combined scorer with subject template assistance in the Nakanishi-style family.",
    ...
    "ecca_paper": "Extended CCA scorer with paper-aligned multi-term reference/template fusion.",
...
def parse_freqs(raw: str) -> tuple[float, ...]:
    values = tuple(float(part.strip()) for part in str(raw).split(",") if part.strip())
    if len(values) < 2:
        raise ValueError("freqs must contain at least two comma-separated values")
    return values
...
class ThresholdProfile:
    ...
    enter_p_th: Optional[float] = None
    exit_p_th: Optional[float] = None
    ...
    confidence_variant: Optional[str] = None
    training_window_policy: Optional[str] = None
...
class FBCCAEngine:
    def __init__(
        ...
        if len(self.freqs) < 2:
            raise ValueError("FBCCAEngine expects at least two target frequencies")
...
def scores_to_feature_dict(scores: np.ndarray, freqs: Sequence[float]) -> dict[str, Any]:
    ...
    top3_index = int(order[2]) if len(order) > 2 else top2_index
    top4_index = int(order[3]) if len(order) > 3 else top3_index
    top1_score = float(values[top1_index])
    top2_score = float(values[top2_index]) if len(order) > 1 else 0.0
    top3_score = float(values[top3_index]) if len(order) > 2 else top2_score
    top4_score = float(values[top4_index]) if len(order) > 3 else top3_score
    ...
    return {
        ...
        "margin": float(margin),
        "ratio": float(ratio),
        "gap_12": float(top1_score - top2_score),
        "gap_13": float(top1_score - top3_score),
        "gap_14": float(top1_score - top4_score),
        "score_sum": score_sum,
        ...
        rows.append(
            {
                ...
                "ratio": result["ratio"],
                "gap_12": result["gap_12"],
                "gap_13": result["gap_13"],
                "gap_14": result["gap_14"],
...
def _extract_training_window_bank(
    segment: np.ndarray,
    *,
    win_samples: int,
    step_samples: int,
    ...
    if matrix.shape[0] < int(win_samples):
        raise ValueError(f"segment too short: {matrix.shape[0]} < {win_samples}")
    stride = max(int(step_samples), 1)
    start_min = max(int(latency_samples), 0)
    max_start = max(int(matrix.shape[0]) - int(win_samples), 0)
    if start_min > max_start:
        start_min = max_start
    start_indices = list(range(int(start_min), int(max_start) + 1, stride))
    if not start_indices:
        start_indices = [int(max_start)]
    if len(start_indices) > max(int(max_windows), 1):
        sampled_positions = np.linspace(0, len(start_indices) - 1, max(int(max_windows), 1))
        keep_indices = sorted(
            {
                int(np.clip(int(round(position)), 0, len(start_indices) - 1))
                for position in sampled_positions
            }
        )
        start_indices = [start_indices[index] for index in keep_indices]
    windows: list[np.ndarray] = []
    for start in start_indices:
        windows.append(np.ascontiguousarray(matrix[start : start + int(win_samples), :], dtype=np.float64))
    if not windows:
        windows.append(np.ascontiguousarray(matrix[max_start : max_start + int(win_samples), :], dtype=np.float64))
    return windows
...
def parse_tdca_decoder_variant(value: Optional[str]) -> str:
    variant = str(value or DEFAULT_TDCA_DECODER_VARIANT).strip().lower()
    if variant not in TDCA_DECODER_VARIANTS:
        raise ValueError(f"unsupported tdca decoder_variant: {value}")
    return variant
...
class BaseSSVEPDecoder(ABC):
    ...
    def __init__(
        ...
        *,
        ...
        self.freqs = tuple(float(freq) for freq in freqs)
        if len(self.freqs) < 2:
            raise ValueError(f"{self.model_name} expects at least two target frequencies")
        self.win_sec = float(win_sec)
        self.step_sec = float(step_sec)
...
class TemplateCCADecoder(BaseSSVEPDecoder):
    def __init__(
        ...
        *,
        ...
        self.reference_template_corrs: dict[float, float] = {}
    ...
    def fit(self, trial_segments: Sequence[tuple[TrialSpec, np.ndarray]]) -> None:
        ...
        reference_template_corrs: dict[float, float] = {}
        for freq in self.freqs:
            ...
            reference_template_corrs[float(freq)] = float(
                self._core.cca_multi_channel_svd(template, self._core.Y_refs[float(freq)])
            )
            try:
        ...
        self.reference_template_corrs = reference_template_corrs
    ...
    def score_window(self, X_window: np.ndarray) -> np.ndarray:
        ...
        for index, freq in enumerate(self.freqs):
            ...
            if self.variant == "itcca":
                ...
                continue
            if self.variant == "cca_itcca_combo":
                scores[index] = float(max(r_ref, 0.0) ** 2 + max(r_template, 0.0) ** 2)
                continue
            ...
            if self.variant == "ecca_paper":
                r_ref_template = float(self.reference_template_corrs.get(float(freq), 0.0))
                scores[index] = float(
                    max(r_ref, 0.0) ** 2
                    + max(r_template, 0.0) ** 2
                    + max(r_proj, 0.0) ** 2
                    + max(r_ref_template, 0.0) ** 2
                )
                continue
    ...
    def get_state(self) -> dict[str, Any]:
        return {
            ...
            "reference_template_corrs": {
                f"{freq:g}": float(value) for freq, value in self.reference_template_corrs.items()
            },
        }
    ...
    def set_state(self, state: Optional[dict[str, Any]]) -> None:
        ...
        self.reference_template_corrs = {
            float(str(key).replace("Hz", "")): float(value)
            for key, value in dict(payload.get("reference_template_corrs", {}) or {}).items()
        }
...
class TDCADecoder(BaseSSVEPDecoder):
    def __init__(
        ...
        *,
        ...
        self.decoder_variant = parse_tdca_decoder_variant(params.get("decoder_variant"))
        default_analysis_latency_sec = (
            DEFAULT_TDCA_TRAINING_LATENCY_SEC if self.decoder_variant == "tdca_paper_aligned" else 0.0
        )
        self.analysis_latency_sec = max(
            0.0,
            float(
                params.get(
                    "analysis_latency_sec",
                    params.get("training_latency_sec", default_analysis_latency_sec),
                )
            ),
        )
        self.training_latency_sec = float(self.analysis_latency_sec)
        nh = int(self.model_params.get("Nh", DEFAULT_NH))
        self.model_params["decoder_variant"] = str(self.decoder_variant)
        self.model_params["analysis_latency_sec"] = float(self.analysis_latency_sec)
        self.model_params["training_latency_sec"] = float(self.training_latency_sec)
        self.training_window_policy = (
            "banked_latency_trimmed_uniform_trial_equal"
            if self.decoder_variant == "tdca_paper_aligned"
            else "last_window_only"
        )
        self.model_params["training_window_policy"] = str(self.training_window_policy)
        self._core = FBCCAEngine(
            sampling_rate=sampling_rate,
            freqs=freqs,
            win_sec=win_sec,
        ...
        self.band_templates: list[dict[float, np.ndarray]] = []
        self.reference_q_factors: list[dict[float, np.ndarray]] = []
        self.paper_alignment_level = "near-paper" if self.decoder_variant == "tdca_paper_aligned" else "partial"
        self.analysis_win_sec = float(self.win_sec)
        self.analysis_win_samples = int(self._core.win_samples)
        self.analysis_latency_samples = int(round(float(self.analysis_latency_sec) * float(self.fs)))
        self.effective_raw_win_samples = int(self.analysis_win_samples + self.analysis_latency_samples)
        self.effective_raw_window_sec = float(self.effective_raw_win_samples / float(self.fs))
        if self.decoder_variant == "tdca_paper_aligned":
            self.win_samples = int(self.effective_raw_win_samples)
    ...
    def _prepare_analysis_window(self, window: np.ndarray) -> np.ndarray:
        matrix = np.asarray(window, dtype=float)
        if matrix.ndim != 2:
            raise ValueError("window must have shape (samples, channels)")
        if self.decoder_variant != "tdca_paper_aligned":
            if matrix.shape[0] < int(self.analysis_win_samples):
                raise ValueError(
                    f"tdca window too short: {matrix.shape[0]} < {self.analysis_win_samples}"
                )
            return np.ascontiguousarray(matrix[-int(self.analysis_win_samples) :, :], dtype=np.float64)
        if matrix.shape[0] >= int(self.effective_raw_win_samples):
            raw = np.ascontiguousarray(matrix[-int(self.effective_raw_win_samples) :, :], dtype=np.float64)
            start = int(self.analysis_latency_samples)
            stop = int(start + self.analysis_win_samples)
            return np.ascontiguousarray(raw[start:stop, :], dtype=np.float64)
        if matrix.shape[0] >= int(self.analysis_win_samples):
            return np.ascontiguousarray(matrix[-int(self.analysis_win_samples) :, :], dtype=np.float64)
        raise ValueError(
            f"tdca paper-aligned window too short: {matrix.shape[0]} < {self.analysis_win_samples}"
        )

    def _augment_delay(self, matrix: np.ndarray) -> np.ndarray:
        return _augment_delay_matrix(matrix, self.delay_steps)

    def _build_tdca_reference_q(self, *, freq: float, time_length: int) -> np.ndarray:
        reference = self._core.build_ref_matrix(
            int(self.fs),
            int(time_length),
            float(freq),
            int(self.model_params.get("Nh", DEFAULT_NH)),
        )
        q_factor, _ = np.linalg.qr(np.asarray(reference, dtype=float), mode="reduced")
        return np.asarray(q_factor, dtype=float)

    @staticmethod
    def _serialize_frequency_matrix_map(payload: Mapping[float, np.ndarray]) -> dict[str, Any]:
        return {f"{float(freq):g}": np.asarray(matrix, dtype=float).tolist() for freq, matrix in payload.items()}

    @staticmethod
    def _deserialize_frequency_matrix_map(payload: Sequence[Mapping[str, Any]]) -> list[dict[float, np.ndarray]]:
        return [
            {float(str(key).replace("Hz", "")): np.asarray(value, dtype=float) for key, value in dict(entry).items()}
            for entry in payload
        ]

    def _fit_legacy_variant(self, grouped: Mapping[float, list[np.ndarray]]) -> None:
        ...
        self.band_templates = band_templates
        self.reference_q_factors = [{} for _ in band_projection]

    def _fit_paper_aligned_variant(self, grouped: Mapping[float, list[list[np.ndarray]]]) -> None:
        band_projection: list[np.ndarray] = []
        band_templates: list[dict[float, np.ndarray]] = []
        reference_q_factors: list[dict[float, np.ndarray]] = []
        for coeffs in self._core.subband_filters:
            class_trials: dict[float, list[np.ndarray]] = {}
            class_means: dict[float, np.ndarray] = {}
            q_factors_for_band: dict[float, np.ndarray] = {}
            for freq in self.freqs:
                trial_window_banks = list(grouped[float(freq)])
                if not trial_window_banks:
                    raise ValueError(f"{self.model_name} fit missing delayed trials for frequency {freq}")
                sample_window_bank = [self._core.bandpass_filter_multichannel(item, coeffs) for item in trial_window_banks[0]]
                delayed_sample_windows = [self._augment_delay(item).T for item in sample_window_bank]
                if not delayed_sample_windows:
                    raise ValueError(f"{self.model_name} fit missing delayed trials for frequency {freq}")
                time_length = int(delayed_sample_windows[0].shape[1])
                q_factor = self._build_tdca_reference_q(freq=float(freq), time_length=time_length)
                q_factors_for_band[float(freq)] = q_factor
                augmented_trials: list[np.ndarray] = []
                for trial_bank in trial_window_banks:
                    filtered_windows = [self._core.bandpass_filter_multichannel(item, coeffs) for item in trial_bank]
                    delayed_windows = [self._augment_delay(item).T for item in filtered_windows]
                    if not delayed_windows:
                        continue
                    augmented_windows = []
                    for delayed in delayed_windows:
                        projected = delayed @ q_factor @ q_factor.T
                        augmented_windows.append(np.concatenate([delayed, projected], axis=0))
                    augmented_trials.append(np.mean(np.stack(augmented_windows, axis=0), axis=0))
                if not augmented_trials:
                    raise ValueError(f"{self.model_name} fit missing augmented trials for frequency {freq}")
                class_trials[float(freq)] = augmented_trials
                class_means[float(freq)] = np.mean(np.stack(augmented_trials, axis=0), axis=0)
            ...
            band_projection.append(projector)
            band_templates.append(class_means)
            reference_q_factors.append(q_factors_for_band)
        self.band_projection = band_projection
        self.band_templates = band_templates
        self.reference_q_factors = reference_q_factors
    ...
    def fit(self, trial_segments: Sequence[tuple[TrialSpec, np.ndarray]]) -> None:
        grouped: dict[float, list[np.ndarray]] = defaultdict(list)
        grouped_trial_banks: dict[float, list[list[np.ndarray]]] = defaultdict(list)
        for trial, segment in trial_segments:
            if trial.expected_freq is None:
                continue
            weighted_segment = self._apply_channel_weights(np.asarray(segment, dtype=float))
            if self.decoder_variant == "tdca_paper_aligned":
                available_latency_samples = max(
                    min(
                        int(self.analysis_latency_samples),
                        int(weighted_segment.shape[0]) - int(self.analysis_win_samples),
                    ),
                    0,
                )
                raw_bank_win_samples = int(self.analysis_win_samples + available_latency_samples)
                training_windows = _extract_training_window_bank(
                    weighted_segment,
                    win_samples=int(raw_bank_win_samples),
                    step_samples=int(self._core.step_samples),
                    latency_samples=0,
                    max_windows=4,
                )
            else:
                training_windows = [_extract_last_window(weighted_segment, self.analysis_win_samples)]
            processed_windows = [
                self._core.preprocess_window(self._prepare_analysis_window(window))
                for window in training_windows
            ]
            if self.decoder_variant == "tdca_paper_aligned":
                grouped_trial_banks[float(trial.expected_freq)].append(processed_windows)
            else:
                grouped[float(trial.expected_freq)].extend(processed_windows)
        grouped_source: Mapping[float, list[Any]] = grouped_trial_banks if self.decoder_variant == "tdca_paper_aligned" else grouped
        if not all(grouped_source.get(float(freq)) for freq in self.freqs):
            missing = [freq for freq in self.freqs if not grouped_source.get(float(freq))]
            raise ValueError(f"{self.model_name} fit missing training trials for frequencies: {missing}")
        self.training_window_policy = (
            "banked_latency_trimmed_uniform_trial_equal"
            if self.decoder_variant == "tdca_paper_aligned"
        ...
        self.model_params["training_latency_sec"] = float(self.training_latency_sec)
        if self.decoder_variant == "tdca_paper_aligned":
            self._fit_paper_aligned_variant(grouped_trial_banks)
        else:
            self._fit_legacy_variant(grouped)

    def score_window(self, X_window: np.ndarray) -> np.ndarray:
        if not self.band_projection or not self.band_templates:
            raise RuntimeError(f"{self.model_name} requires fit() before score_window()")
        base = self._core.preprocess_window(self._prepare_analysis_window(np.asarray(X_window, dtype=float)))
        scores = np.zeros(len(self.freqs), dtype=float)
        for band_index, coeffs in enumerate(self._core.subband_filters):
            band_signal = self._core.bandpass_filter_multichannel(base, coeffs)
            delayed = self._augment_delay(band_signal).T
            ...
            templates = self.band_templates[band_index]
            for class_index, freq in enumerate(self.freqs):
                template_matrix = templates[float(freq)]
                if self.decoder_variant == "tdca_paper_aligned":
                    q_payload = self.reference_q_factors[band_index][float(freq)]
                    window_matrix = np.concatenate([delayed, delayed @ q_payload @ q_payload.T], axis=0)
                else:
                    window_matrix = delayed
                projected_window = projector.T @ window_matrix
                projected_template = projector.T @ template_matrix
                component_scores = [
    ...
    def iter_window_features(
        ...
        *,
        ...
        if self.decoder_variant != "tdca_paper_aligned":
            return super().iter_window_features(
                segment,
                expected_freq=expected_freq,
                label=label,
                trial_id=trial_id,
        ...
        if segment_matrix.ndim != 2:
            raise ValueError("segment must have shape (samples, channels)")
        if segment_matrix.shape[0] < int(self.effective_raw_win_samples):
            raise ValueError("segment is shorter than the paper-aligned raw analysis window")
        rows: list[dict[str, Any]] = []
        for window_index, start in enumerate(
            range(0, segment_matrix.shape[0] - self.effective_raw_win_samples + 1, self.step_samples)
        ):
            raw_window = np.ascontiguousarray(
                segment_matrix[start : start + self.effective_raw_win_samples],
                dtype=np.float64,
            )
            result = self.analyze_window(raw_window)
            correct = expected_freq is not None and abs(float(result["pred_freq"]) - float(expected_freq)) < 1e-8
            rows.append(
                {
                    "label": label,
    ...
    def get_state(self) -> dict[str, Any]:
        return {
            "decoder_variant": str(self.decoder_variant),
            "analysis_latency_sec": float(self.analysis_latency_sec),
            "effective_raw_window_sec": float(self.effective_raw_window_sec),
            "paper_alignment_level": str(self.paper_alignment_level),
            "training_window_policy": str(self.training_window_policy),
            "training_latency_sec": float(self.training_latency_sec),
            "delay_steps": int(self.delay_steps),
            "n_components": int(self.n_components),
            "band_projection": [np.asarray(matrix, dtype=float).tolist() for matrix in self.band_projection],
            "band_templates": [
                self._serialize_frequency_matrix_map(entry)
                for entry in self.band_templates
            ],
            "reference_q_factors": [
                self._serialize_frequency_matrix_map(entry)
                for entry in self.reference_q_factors
            ],
        }

    def set_state(self, state: Optional[dict[str, Any]]) -> None:
        payload = dict(state or {})
        self.decoder_variant = parse_tdca_decoder_variant(payload.get("decoder_variant", self.decoder_variant))
        self.model_params["decoder_variant"] = str(self.decoder_variant)
        default_analysis_latency_sec = (
            DEFAULT_TDCA_TRAINING_LATENCY_SEC if self.decoder_variant == "tdca_paper_aligned" else 0.0
        )
            ...
            float(
                payload.get(
                    ...
                    payload.get(
                        ...
                        self.analysis_latency_sec if hasattr(self, "analysis_latency_sec") else default_analysis_latency_sec,
                    ),
                )
            ),
        )
        ...
        self.model_params["analysis_latency_sec"] = float(self.analysis_latency_sec)
        self.model_params["training_latency_sec"] = float(self.training_latency_sec)
        self.training_window_policy = str(
            payload.get(
        ...
        self.model_params["training_window_policy"] = str(self.training_window_policy)
        self.paper_alignment_level = str(
            payload.get(
                "paper_alignment_level",
                "near-paper" if self.decoder_variant == "tdca_paper_aligned" else "partial",
            )
        )
        ...
        templates_raw = list(payload.get("band_templates", []))
        q_raw = list(payload.get("reference_q_factors", []))
        self.band_projection = [np.asarray(matrix, dtype=float) for matrix in projection_raw]
        self.band_templates = self._deserialize_frequency_matrix_map(templates_raw)
        self.reference_q_factors = self._deserialize_frequency_matrix_map(q_raw)
        self.analysis_win_sec = float(self.win_sec)
        self.analysis_win_samples = int(self._core.win_samples)
        self.analysis_latency_samples = int(round(float(self.analysis_latency_sec) * float(self.fs)))
        self.effective_raw_win_samples = int(self.analysis_win_samples + self.analysis_latency_samples)
        self.effective_raw_window_sec = float(
            payload.get(
                "effective_raw_window_sec",
                self.effective_raw_win_samples / max(float(self.fs), 1.0),
            )
        )
        if self.decoder_variant == "tdca_paper_aligned":
            self.win_samples = int(self.effective_raw_win_samples)
...
MODEL_ALIASES = {
    ...
    "cca": "cca",
    "cca_itcca_combo": "cca_itcca_combo",
    "cca-itcca-combo": "cca_itcca_combo",
    "cca_itcca_combo_all8": "cca_itcca_combo",
    "cca-itcca-combo-all8": "cca_itcca_combo",
    "itcca": "itcca",
    "ecca": "ecca",
    "ecca_paper": "ecca_paper",
    "ecca-paper": "ecca_paper",
    "ecca_paper_all8": "ecca_paper",
    "ecca-paper-all8": "ecca_paper",
    "msetcca": "msetcca",
    "trca": "trca",
    "trca_all8": "trca",
    "trca-all8": "trca",
...
def create_decoder(
    ...
    *,
    ...
    decoder_compute_backend: Optional[str] = None,
    gpu_device: int = DEFAULT_GPU_DEVICE_ID,
    gpu_precision: str = DEFAULT_GPU_PRECISION_NAME,
    gpu_warmup: bool = True,
    ...
    if name in {
        ...
        name = "fbcca"
    requested_backend = (
        parse_compute_backend_name(decoder_compute_backend)
        if decoder_compute_backend is not None
        else parse_compute_backend_name(compute_backend)
    ...
    if name in {"itcca", "ecca", "msetcca", "cca_itcca_combo", "ecca_paper"}:
...
def load_decoder_from_profile(
    ...
    *,
    ...
    decoder = create_decoder(
        ...
        model_params=model_params,
        decoder_compute_backend=requested_backend,
...
def build_feature_rows_with_decoder(
    ...
    rows: list[dict[str, Any]] = []
    for trial, segment in trial_segments:
        for feature_row in decoder.iter_window_features(
            segment,
            expected_freq=trial.expected_freq,
            label=trial.label,
            trial_id=trial.trial_id,
            ...
            row["expected_freq"] = trial.expected_freq
            row["label"] = str(trial.label)
            row["trial_id"] = int(trial.trial_id)
            row["block_index"] = int(trial.block_index)
