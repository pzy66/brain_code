...
def _candidate_cuda_bin_dirs() -> list[Path]:
    ...
    def _candidate_python_cuda_roots() -> list[Path]:
    ...
    for package_root in _candidate_python_cuda_roots():
        for subdir in (package_root / "bin", package_root / "lib" / "x64"):
            if subdir.exists():
                _push(subdir)
...
def _prepare_cuda_runtime_env() -> None:
    if not os.environ.get("CUDA_PATH", "").strip():
        ...
        if not os.environ.get("CUDA_PATH", "").strip():
            for key in ("purelib", "platlib"):
                ...
                if not raw:
                    continue
                cuda_runtime_root = Path(raw) / "nvidia" / "cuda_runtime"
                if (cuda_runtime_root / "include" / "cuda.h").exists():
    ...
    for bin_dir in _candidate_cuda_bin_dirs():
        ...
        if str(bin_dir).lower() not in current_lower:
            ...
            current_lower.add(str(bin_dir).lower())


def _runtime_root_dir() -> Path:
    return Path(__file__).resolve().parents[1] / "artifacts" / "gpu_runtime"
