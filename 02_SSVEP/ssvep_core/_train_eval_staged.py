...
from .run_artifacts import publish_deployed_profile, sanitize_artifact_token
...
def _named_report_stem(
    *,
    ...
    _ = subject_id
    rec = sanitize_artifact_token(recommended_model, fallback="model")[:18]
    dep = sanitize_artifact_token(deployed_model, fallback="model")[:18]
    mode = sanitize_artifact_token(channel_mode, fallback="mode")[:12]
    return (
        "summary"
        f"__rec-{rec}"
        f"__dep-{dep}"
...
def run_offline_train_eval(
    ...
    *,
    ...
    resolved_profile_path = Path(resolved_report_paths["output_profile"]).expanduser().resolve()
    resolved_profile_v2_path = Path(resolved_report_paths["profile_v2"]).expanduser().resolve()
    run_tag = str(resolved_report_paths.get("run_tag", report_dir.name))
    ...
    helper = BenchmarkRunner(
        ...
        output_profile_path=resolved_profile_path,
    ...
    if bool(deployment_run.get("meets_acceptance", False)):
        ...
        if chosen_profile is not None:
            save_profile(chosen_profile, resolved_profile_path)
            profile_saved = True
        ...
        try:
            ...
            if profile_saved:
                publish_deployed_profile(
                    ...
                    run_dir=report_dir,
                    task=str(getattr(config, "task", "")) or "train-eval",
                    run_tag=run_tag,
    ...
    named_profile_path = report_dir / (
        "profile"
        f"__model-{sanitize_artifact_token(str(deployment_run.get('model_name', '')), fallback='model')}"
        f"__m-{sanitize_artifact_token(str(deployment_run.get('channel_mode', '')), fallback='mode')}"
    ...
    report_payload["named_artifacts"] = {
        ...
        "default_profile": str(resolved_profile_path) if profile_saved else "",
    }
    ...
    config_payload["output_profile_path"] = str(resolved_profile_path)
    ...
    config_payload["resolved_profile_v2_path"] = str(resolved_profile_v2_path)
    ...
    if profile_saved:
        log(f"Profile saved: {resolved_profile_path}")
    ...
    if profile_saved:
        log(
            ...
            f"{profile_for_realtime_source if profile_for_realtime_source is not None else resolved_profile_path}"
        )
    else:
