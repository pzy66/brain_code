...
def test_discover_collection_manifests_deduplicates_nested_duplicate_session() -> None:
    tmp_root = PROJECT_DIR / ".tmp_test_artifacts" / f"discover_dedup_{uuid.uuid4().hex}"
    outer_dir = tmp_root / "session_dup"
    nested_dir = outer_dir / "session_dup"
    outer_dir.mkdir(parents=True, exist_ok=True)
    nested_dir.mkdir(parents=True, exist_ok=True)
    try:
        np.savez_compressed(outer_dir / "raw_trials.npz", trial_collection_1=np.zeros((100, 8), dtype=np.float32))
        np.savez_compressed(nested_dir / "raw_trials.npz", trial_collection_1=np.zeros((100, 8), dtype=np.float32))
        manifest = {
            "data_schema_version": "2.0",
            ...
            "freqs": [8.0, 10.0, 12.0, 15.0],
            "board_eeg_channels": [0, 1, 2, 3, 4, 5, 6, 7],
            "protocol_config": {"preset_name": "stable_12m", "round_index": 1, "rounds_planned": 1},
            "protocol_signature": "sha1:testsig",
            "quality_summary": {"valid_trial_count": 1, "short_segment_excluded": 0, "retry_total": 0},
            "trials": [
                {
            ...
            "files": {"raw_trials_npz": str(outer_dir / "raw_trials.npz")},
        }
        outer_manifest = outer_dir / "session_manifest.json"
        nested_manifest = nested_dir / "session_manifest.json"
        outer_manifest.write_text(json.dumps(manifest, ensure_ascii=False), encoding="utf-8")
        nested_manifest.write_text(json.dumps(manifest, ensure_ascii=False), encoding="utf-8")

        rows = discover_collection_manifests(tmp_root)

        assert len(rows) == 1
        assert Path(str(rows[0]["manifest_path"])).resolve() == outer_manifest.resolve()
    finally:
        shutil.rmtree(tmp_root, ignore_errors=True)
...
def test_train_eval_organize_report_dir_with_selected_manifests(monkeypatch) -> None:
    ...
    try:
        ...
        for target_module in (module, staged_module):
            ...
            monkeypatch.setattr(target_module, "save_profile", lambda _profile, _path: None)
