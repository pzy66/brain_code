...
from hybrid_controller.adapters.robot_client import RobotClient, fetch_robot_status, parse_robot_line
...
def test_robot_client_healthcheck_with_fake_server() -> None:
    config = AppConfig(timing_profile="fast", sim_pick_delay_sec=0.01).resolved()
    world = SimulationWorld(config, scenario_name="basic")
    server = FakeRobotServer("127.0.0.1", 0, world)
    server.start()
    client = RobotClient("127.0.0.1", server.port, timeout_sec=0.2)
    try:
        client.connect()
        assert client.healthcheck(timeout_sec=1.0) is True
...
def test_fetch_robot_status_returns_fake_server_snapshot() -> None:
    config = AppConfig(timing_profile="fast").resolved()
    world = SimulationWorld(config, scenario_name="basic")
    server = FakeRobotServer("127.0.0.1", 0, world)
    server.start()
    try:
        world.handle_move(60.0, -120.0)
        status = fetch_robot_status("127.0.0.1", server.port, timeout_sec=1.0)
        assert status["state"] == "MOVING_XY"
        assert "robot_xy" in status
        assert status["carry_z"] == 160.0
    finally:
        server.stop()
