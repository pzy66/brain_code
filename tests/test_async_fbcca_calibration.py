...
def test_profile_roundtrip_preserves_gate_behavior() -> None:
    ...
    profile_path = MODULE_PATH.parents[2] / f".tmp_async_fbcca_profile_{uuid.uuid4().hex}.json"
    try:
...
def test_load_profile_can_require_existing_file() -> None:
    module = load_module()
    missing = MODULE_PATH.parents[2] / f".tmp_missing_{uuid.uuid4().hex}.json"
    try:
        try:
...
def test_save_profile_sanitizes_nan_metadata() -> None:
    ...
    profile_path = MODULE_PATH.parents[2] / f".tmp_async_fbcca_nan_{uuid.uuid4().hex}.json"
    try:
        module.save_profile(profile, profile_path)
