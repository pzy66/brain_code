...
from hybrid_controller.integrations.robot_protocol import RobotExecutorState
from hybrid_controller.integrations.robot_runtime import RobotRuntime, build_arg_parser
...
def _patch_executor_sleep(monkeypatch) -> None:
    monkeypatch.setattr(
        "hybrid_controller.integrations.robot_runtime.RobotExecutor._sleep_with_abort",
...
def test_pick_world_rejects_out_of_workspace() -> None:
    ...
    assert stream.lines() == ["ERR target_out_of_workspace: Target exceeds workspace by 160.00 mm."]
