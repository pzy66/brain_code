...
- `22`：SSH，只用于部署、启动、现场诊断和必要修复
- `9091`：`rosbridge`，桌面端 GUI 通过它调用 ROS 服务和订阅状态
- `8080`：`web_video_server`，桌面端相机画面来源
- `11311`：ROS master，JetMax ROS1 基础服务；桌面端主流程不直接依赖它
- `8888`：TCP 兼容链路（可选，仅诊断/回退，当前主流程不需要）

说明：
- 现在没有业务代码依赖 `9092`。
- `8888` 不影响 ROS 主链启动；只有你显式要求时才会检查 TCP 端口。
- `80`、`111`、`4000`、`9026`、`2947` 以及 ROS 节点产生的三万到四万段随机监听端口属于 JetMax 出厂系统、NoMachine/GPSD/rpcbind 或 ROS1 XMLRPC 内部端口；它们不是上位机抓取流程的固定依赖。
- 上位机代码不要硬编码 ROS1 的随机 XMLRPC 端口。主链通过 `9091` rosbridge 和 `8080` 相机流完成。
