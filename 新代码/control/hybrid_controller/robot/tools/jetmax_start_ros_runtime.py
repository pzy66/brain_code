...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--camera-video-device",
        default="auto",
        help="Camera device for usb_cam_node. Use auto to choose /dev/usb_cam0, /dev/video0, or the first /dev/video* present.",
    )
...
def repair_official_camera_sender(ssh: paramiko.SSHClient, *, args: argparse.Namespace) -> None:
    video_device = resolve_camera_video_device(ssh, requested=str(args.camera_video_device))
    config = {
    ...
    print(f"camera video device: {video_device}")
...
def resolve_camera_video_device(ssh: paramiko.SSHClient, *, requested: str = "auto") -> str:
    value = str(requested or "auto").strip()
    if value and value.lower() != "auto":
        return value
    listing = run_remote_command(
        ssh,
        "for p in /dev/usb_cam0 /dev/video0 /dev/video1 /dev/video2 /dev/video3; do "
    ...
    if not candidates:
        raise RuntimeError("No JetMax camera device found under /dev/usb_cam0 or /dev/video*.")
    return candidates[0]
