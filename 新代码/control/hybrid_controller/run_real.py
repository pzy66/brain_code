...
def _candidate_brain_code_pythons() -> tuple[Path, ...]:
    ...
    for root_name in ("miniconda3", "anaconda3", "mambaforge"):
        candidates.append(home / root_name / "envs" / "brain_robot" / venv_dir / python_parts[0])
...
def _enforce_brain_code_interpreter() -> None:
    ...
    if override:
        ...
        if not override_path.exists():
            raise SystemExit(
                ...
                "Please fix BRAIN_PYTHON_EXE or switch PyCharm interpreter to the repo .venv or brain_robot environment."
            )
    ...
    raise SystemExit(
        ...
        f"Current: {current}\n"
        f"Expected one of:\n{expected_text}\n"
        "Please switch PyCharm interpreter to the repo .venv or brain_robot environment and run again.\n"
