...
class JetMaxExecutor(object):
    def __init__(self, limits, home_pose):
        ...
        self.radius_limits = tuple(float(value) for value in limits.get("radius_limits", (50.0, 230.0)))
    ...
    def _cyl_start_move_auto_impl(self, sender, theta_deg, radius_mm):
        effective_limits = nearest_profile_radius_limits(self._auto_z_profile)
    ...
    def _legacy_start_pick_world_impl(self, sender, target_x, target_y):
        ...
        if not self._within_workspace(target_x, target_y):
            return format_error_line(
                ERR_TARGET_OUT_OF_WORKSPACE,
                "Target ({0:.2f}, {1:.2f}) is outside workspace.".format(target_x, target_y),
            )
        with self._lock:
            if self._busy:
                return "BUSY"
...
class RobotGateway(object):
    ...
    def handle(self, line, sender):
        ...
        try:
            ...
            if command == "MOVE":
                if len(parts) != 3:
                    raise RobotCommandError(ERR_INVALID_STATE, "MOVE requires x y.")
                return self.executor.legacy_kernel.start_move(sender, float(parts[1]), float(parts[2]))
            if command == "MOVE_CYL":
                if len(parts) != 4:
                    raise RobotCommandError(ERR_INVALID_STATE, "MOVE_CYL requires theta r z.")
                return self.executor.cylindrical_kernel.start_move(sender, float(parts[1]), float(parts[2]), float(parts[3]))
            if command == "MOVE_CYL_AUTO":
                if len(parts) != 3:
                    raise RobotCommandError(ERR_INVALID_STATE, "MOVE_CYL_AUTO requires theta r.")
                return self.executor.cylindrical_kernel.start_move_auto(sender, float(parts[1]), float(parts[2]))
            if command == "PICK":
                if len(parts) != 3:
                    raise RobotCommandError(ERR_INVALID_STATE, "PICK requires u v.")
                return self.executor.legacy_kernel.start_pick_pixel(sender, float(parts[1]), float(parts[2]))
            if command == "PICK_WORLD":
                if len(parts) != 3:
                    raise RobotCommandError(ERR_INVALID_STATE, "PICK_WORLD requires x y.")
                return self.executor.legacy_kernel.start_pick_world(sender, float(parts[1]), float(parts[2]))
            if command == "PICK_CYL":
                if len(parts) != 3:
                    raise RobotCommandError(ERR_INVALID_STATE, "PICK_CYL requires theta r.")
                return self.executor.cylindrical_kernel.start_pick(sender, float(parts[1]), float(parts[2]))
            if command == "PLACE":
                if len(parts) != 1:
                    raise RobotCommandError(ERR_INVALID_STATE, "PLACE takes no args.")
