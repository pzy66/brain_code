...
class MainController(QWidget):
    ...
    def keyPressEvent(self, event):
        ...
        if self.state == STATE_SEARCH:
            ...
            target_idx = -1
            if key == Qt.Key_1: target_idx = 0
            elif key == Qt.Key_2: target_idx = 1
            elif key == Qt.Key_3: target_idx = 2
            elif key == Qt.Key_4: target_idx = 3
    ...
    def paintEvent(self, event):
        ...
        if self.state == STATE_SEARCH:
            for t in self.stim_targets:
                ...
                # 不同频率不同色调微调
                if freq == 8: color = QColor(255, 0, 0, alpha)
                elif freq == 10: color = QColor(0, 255, 0, alpha)
                elif freq == 12: color = QColor(0, 0, 255, alpha)
